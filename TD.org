...
#+BEGIN_SRC c++
  void createAndWait(int N) {
	  ...
	  // Dans sa version foreach
	  for (auto &t : threads) {
		  t.join();
		  cout << "Joined " << ++i;
		  // Oh ben merde, on n'a pas créé i !
		  // "On aurait peut-être dû rester sur une boucle for simple"
		  // Yann Thierry-Mieg, 2019
		  // Si on avait besoin de preuve supplémentaire que toutes les
		  // "fonctionnalités" C++ sont principalement faites pour vous
		  // faire vous casser la gueule
	  }
...
Il y a plein d'exécutions possibles (Combien ?? [à refaire en exercice maison])
...
Question typiquement c++ : on peut faire comme ça, mais "c'est trop dangereux". Implémentons 15 manières différentes de faire la même chose, noyons le langage sous 1 tonne de sucre syntaxique, what could possibly go wrong ?
...
A dire vrai, la vraie bonne manière, c'est de limiter la quantité des échappements de la fonction :
